...
// ─────────────────────────────────────────────
server.tool("oracle_validate", `Submit a critical or irreversible action to ORACLE-H for human approval.
...
Returns a request_id to use with oracle_poll_status to retrieve the decision.`, {
    action: zod_1.z.string().min(15).describe("Precise technical description of the action you are about to execute"),
    rationale: zod_1.z.string().min(15).describe("Why this action is necessary and why it is the optimal path"),
    projected_impact: zod_1.z.string().min(15).describe("Risk and impact if this action fails or is wrong. Must describe potential damage."),
    financial_amount: zod_1.z.number().min(0).default(0).describe("Financial amount at stake in USD (0 if none)"),
    expires_in_seconds: zod_1.z.number().min(30).default(300).describe("How long to wait for the human response before timing out (default: 5 minutes)"),
...
// ────────────────────────────────────────────────────
server.tool("oracle_poll_status", `Check the current status of a validation request previously submitted via oracle_validate.
...
If "expired", the human did not respond in time — abort the action.`, {
    request_id: zod_1.z.string().describe("The request_id returned by oracle_validate"),
}, async ({ request_id }) => {
    try {
        ...
        const decisionTimestamp = data.decision_timestamp;
        const emoji = status === "approved" ? "✅" : status === "rejected" ? "🚫" : status === "expired" ? "⌛" : "⏳";
        const instruction = status === "approved" ? "The human approved the action. You may now proceed." :
...
// ────────────────────────────────────────────────────
server.tool("oracle_cancel", "Cancel a pending validation request submitted via oracle_validate. Use this when the context has changed and the original action is no longer needed, or when you want to free up a pending request. Only works on requests still in 'pending' status.", {
    request_id: zod_1.z.string().describe("The request_id to cancel"),
}, async ({ request_id }) => {
    try {
...
// ────────────────────────────────────────────────────
server.tool("oracle_list_pending", "List all your pending validation requests that are still awaiting human response. Use this to recover state after a crash or restart, or to check if you have active requests before submitting a new one.", {}, async () => {
    try {
...
server.tool("oracle_wait", "Submit an action for human approval AND wait for the decision in a single call. This is the preferred alternative to calling oracle_validate + oracle_poll_status separately. The tool blocks until the human decides or the request expires. Use this for simple workflows where you want to submit and wait in one step.", {
    ...
    while (Date.now() - startTime < maxWaitMs) {
        ...
        try {
            ...
            const decisionTimestamp = pollData.decision_timestamp;
            if (status === "pending") {
                continue;
            }
